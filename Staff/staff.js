...
/* ═══════════════════════════════════
   NAVIGATION
═══════════════════════════════════ */
const pageTitles={menu:"Menu",orders:"Orders",dashboard:"Dashboard",archive:"Archive"};
navBtns.forEach(btn=>{
...
function openAvailDropdown(id, card, anchorBtn){
  ...
  drop.querySelector('[data-action="restock"]')?.addEventListener('mousedown',e=>{
    e.preventDefault();
    e.stopPropagation();
    setStock(id,20);
...
function addItem(card){
  ...
  if(ex)ex.qty++;else cart.set(id,{id,name,price,img,qty:1});
...
function finaliseOrder(payment,tendered,total,customer,orderType,persons,sub,tax,_total){
  ...
  // Finish Button Logic
  $("#receiptFinishBtn").addEventListener("click", function() {
    // 1. Deduct stock
    ...
    const order={
        ...
        items:orderItems,sub,tax,total,time:now()
    };
